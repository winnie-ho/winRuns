      ...
      this.orderedSessionEfforts.forEach(effort => {
        ...
        if (type === "Swim") {
          sessionString =
            ...
            index +
            " - " +
            this.renderSwimDistance(effort.distance) +
            ", " +
            this.renderTime(effort.moving_time) +
            " (" +
            this.renderSwimPace(effort.moving_time, effort.distance) +
            ") " +
            rankMark +
            "\n";
        } else {
          sessionString =
            ...
            index +
            "\t - \t" +
            this.renderDistance(effort.distance) +
            ", \t" +
            this.renderTime(effort.moving_time) +
            "\t(" +
            this.renderPace(effort.moving_time, effort.distance) +
            ") " +
            rankMark +
            "\n";
        }
  ...
  computed: {
    ...
    fastestSessionEffortIds() {
      ...
      const fastestArray = this.finalLaps
        .slice()
        .sort(
          (a, b) => a.moving_time / a.distance - b.moving_time / b.distance
        );
      const fastestIds = [];
      for (let i = 0; i < 3; i++) {
